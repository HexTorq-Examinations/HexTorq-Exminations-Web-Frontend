...
export function MessagingPanel() {
  ...
  const {
    isPanelOpen, closePanel, conversations, activeConversationId, messages,
    ...
    <Sheet open={isPanelOpen} onOpenChange={(open) => !open && closePanel()}>
      <SheetContent side="right" className="w-full sm:max-w-2xl p-0 flex flex-col">
        <SheetHeader className="px-4 py-3 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <SheetTitle className="flex items-center gap-2"><MessageSquare className="w-5 h-5" /> Messages</SheetTitle>
        </SheetHeader>
        ...
        <div className="flex-1 flex min-h-0">
          ...
          <div className={`flex-1 flex flex-col min-w-0 ${showChatOnMobile ? 'flex' : 'hidden sm:flex'}`}>
            {composeMode ? (
              ...
              <div className="flex-1 hidden sm:flex items-center justify-center text-slate-400 text-sm">
