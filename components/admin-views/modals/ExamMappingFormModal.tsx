...
import type { ApiErrorWithRows } from '@/lib/api';
...
export function ExamMappingFormModal({ open, onOpenChange, exam, mappingToEdit }: ExamMappingFormModalProps) {
  ...
  useEffect(() => {
    ...
    if (mappingToEdit) {
      ...
      setGraceMinutes(mappingToEdit.graceMinutes || 0);
    } else {
      setSchoolId('');
      setDepartmentId('');
      setClassId('');
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mappingToEdit?.id) {
        await updateExamMapping(mappingToEdit.id, { date, startTime, endTime, hall, graceMinutes, confirmOverlap: overlapPending });
      } else {
        await addExamMapping({ examId: exam.id, classId, date, startTime, endTime, hall, graceMinutes, status: 'Scheduled', confirmOverlap: overlapPending });
      }
      onOpenChange(false);
    } catch (caught) {
      const apiError = caught as ApiErrorWithRows;
      if (apiError.code === 'SCHEDULE_OVERLAP') {
        setOverlapPending(true);
        setError('This class already has an exam during this time. Review the warning, then click Confirm Overlap & Save to continue anyway.');
      }
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[650px]">
        ...
        <div className="space-y-4 mt-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="grace-minutes">Grace Time (Minutes)</Label>
            <Input id="grace-minutes" type="number" min="0" value={graceMinutes} onChange={(e) => setGraceMinutes(Math.max(0, Number(e.target.value) || 0))} />
          </div>
        ...
        <DialogFooter className="mt-4">
          ...
          <Button type="button" onClick={handleSubmit} disabled={isLoading} className="bg-blue-600 hover:bg-blue-700 text-white">
            {isLoading ? 'Saving...' : overlapPending ? 'Confirm Overlap & Save' : 'Save Mapping'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
